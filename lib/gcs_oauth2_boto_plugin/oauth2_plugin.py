...
from ..boto.boto.auth_handler import AuthHandler
from ..boto.boto.auth_handler import NotReadyToAuthenticate
...
class OAuth2ServiceAccountAuth(AuthHandler):
    ...
    def __init__(self, path, config, provider):
        if provider.name == 'google' and config.has_option('Credentials', 'gs_service_key_file'):
